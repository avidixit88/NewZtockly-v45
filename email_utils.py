...
# Identify signal family without relying on any one key.
family = (g('signal_family','SignalFamily','family','Family') or extras.get('family') or '').upper()

# SWING-specific diagnostics (elite pullback entry engine).
# Keep this isolated so other alert families are unaffected.
if family == "SWING":
    swing_stage = extras.get("swing_stage") or g('swing_stage','SwingStage')
    if swing_stage:
        lines.append(f"Swing stage: {swing_stage}")
    trend_lock = extras.get("trend_lock_score")
    if trend_lock is not None:
        lines.append(f"Trend lock: {trend_lock}/5")
    ...
    lines.append("")  # spacer after SWING block
    ...
    # SWING family stores pullback band as a tuple in extras ("pullback_band").
    # We keep this read confined to payload/extras so other alert families are unaffected.
    if (pb1 is None or pb2 is None):
        pb_band = g('pullback_band', 'PullbackBand')
        if pb_band is None:
        ...
        if isinstance(pb_band, (tuple, list)) and len(pb_band) == 2:
            try:
                ...
                # Fall back to string representation if types are non-numeric
    ...
    # SWING-specific enriched fields (safe no-ops for other families)
    ez = g('entry_zone','EntryZone')
    if ez is None:
        ez = extras.get('entry_zone')
    if ez is not None:
        lines.append(f"Entry zone: {ez}")
    rp = g('retrace_pct','RetracePct')
    if rp is None:
        rp = extras.get('retrace_pct')
    pq = g('pullback_quality','PullbackQuality')
    if pq is None:
        pq = extras.get('pullback_quality')
    cc = g('confluence_count','ConfluenceCount')
    if cc is None:
        cc = extras.get('confluence_count')
    if rp is not None or pq is not None or cc is not None:
        lines.append(f"Retrace: {rp}   PB Quality: {pq}   Confluence: {cc}")
